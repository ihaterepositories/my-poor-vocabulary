...
{
    public class TeacherAccountStudentsByAverageScorePerGameSorter : ITeacherAccountStudentsSorter
    {
        public List<StudentCredentials> Sort(List<StudentCredentials> students)
        {
            return students.OrderByDescending(s => s.AverageScorePerGame).ToList();
        }
    }
}